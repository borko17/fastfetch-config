fastfetch -c ~/.config/fastfetch/nixos-01.jsonc
...
if (( cache_mtime < boot_time )); then
  ...
  printf '%s%s┌──────%s───────%s───────%s───────%s───────%s───────%s───────%s───────%s───────%s──────┐%s %sNixOS Upgrade\n' \
    "$pad" "$c1" "$c2" "$c3" "$c4" "$c5" "$c6" "$c7" "$c8" "$c9" "$c10" "$c10"
  ...
  # --- Bottom decoration ---
  printf '%s                %s󱄅 %s󱄅 %s󱄅 %s󱄅 %s󱄅 %s󱄅 %s󱄅 %s󱄅 %s󱄅 %s󱄅%s\n' \
    "$pad" "$c10" "$c9" "$c8" "$c7" "$c6" "$c5" "$c4" "$c3" "$c2" "$c1" "$reset"
  printf '\n'
}
...
alias check='rm /tmp/nixos_update_cache && nixos_latest'
alias upgrade='sudo nixos-rebuild switch --upgrade'
alias garbage='sudo nix-collect-garbage -d && sudo nixos-rebuild switch && reboot'
